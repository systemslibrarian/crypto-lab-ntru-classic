 ...
 *    an unknown backdrop, or a `color-mix()`. THE WHOLE PAGE is one here:
 *    `<body>` has no background-color at all, only three stacked
 *    background-images — `radial-gradient(circle at 10% 10%, #123766 …)`,
 *    `radial-gradient(circle at 90% 20%, #40103d …)` and a
 *    `linear-gradient(180deg, #060911, #090f18)`. Every card sits on top of
 *    that with its own `color-mix(in oklab, var(--panel) 92%, black)`, and the
 *    intro card and hero aside are `color-mix(in srgb, var(--accent) 7%,
 *    var(--panel))`. axe files all of them under `incomplete`, so a
 *    violations-only gate measured the contrast of nothing on this page.
 ...
 *    at `.9`, `button:disabled` at `.6`, and — the one that matters — the
 *    footer, which sets `opacity: .85` inline on the `<footer>` and then
 *    `opacity: .75` inline on the scripture line INSIDE it. Opacity compounds,
 *    so that line paints at an effective 0.6375 and there is no declaration
 ...
 *    boxes are `.log` (`max-height: 15rem; overflow: auto`, which the keygen
 *    log and the LLL readout both overflow once enough attempts or reduction
 *    steps have run), `.scheme-eqs` (`overflow-x: auto`) and `.table-wrap`
 *    (`overflow-x: auto` round the comparison table). Content scrolled past
 ...
 *    every ancestor's box, so the ancestor walk would find nothing behind it
 *    and fall through to white, inventing a failure.
 *
 ...
 * `::after` is not one, nor is it one to axe's `color-contrast` rule.
 * `nontext.ts` measures those separately, from the same paint model.
 *
 * SECOND, and much more important on this page: `<canvas>`. Nine of them —
 * the two key rings, the four encrypt/decrypt rings, the four pipeline cells
 * and the 2D lattice — carry every visual result this lab produces, and a
 * canvas is one opaque element to both oracles. Its pixels are drawn by
 * `main.ts` from hard-coded hex, and nothing automated can reach them. They
 * are hand-measured in `gate.ts`'s header.
 *
 * THIRD, `.ring-legend`. It is `aria-hidden="true"` and it is the colour KEY
 * for those canvases, so both oracles skip it while it is fully painted text
 * with `.swatch` colour chips beside it. Also hand-measured there.
 */
 ...
 * uses. This lab's short-lived styling is the 0.25s `opacity`/`filter`
 * transition on `.eq-step` as an equation unlocks, and the 600-700ms `riseIn`
 * animation on `.cl-hero` and every `.card` — and under the reduced motion this
 * gate asserts, the stylesheet cancels both outright (`animation: none;
 * transition: none`), so there is no half-faded frame to catch.
 *
 * The parameter is kept because the narrowed form is what makes a short-lived
 * state measurable at all: a class that lives a quarter of a second is shorter
 * than a full axe pass, so a whole-page scan would catch it only sometimes, and
 * a sometimes-measured assertion is worse than none.
 */
export async function auditContrast(page: Page, within = 'body *'): Promise<ContrastFailure[]> {
  return page.evaluate((rootSelector) => {
     ...
     *
     * This page is a single long document of five exhibits with a KaTeX-rendered
     * equation list, and KaTeX expands each formula into dozens of nested
     * `<span>`s — so the equation block alone is hundreds of elements all
     * re-walking the same ancestors up to `<body>`, whose three stacked
     * gradients are the most expensive thing in the walk to sample. Without the
     ...
     * that is `.log` (`max-height: 15rem; overflow: auto`), `.scheme-eqs` and
     * `.table-wrap` (both `overflow-x: auto`).
     */
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // paints nothing. `checkVisibility()` catches it. Here that is
      // `#decrypt-walkthrough`, the one `<details>` on the page, which ships
      // shut and holds the step-by-step algebra. The gate opens it by clicking
      // its <summary>, which is the route a reader has, rather than setting
      // `.open` from script — which is what the gate this replaces did, and it
      // did it to a walkthrough that is EMPTY until a message has been
      // decrypted, so what it revealed was a placeholder.
      ...
      // not merely inherited — though on this page the OTHER idiom is what is
      // live. This lab has no `.sr-only`; it parks its own `.skip-link` at
      // `left: -9999px` and slides it in on focus, which the document-space
      // guard above handles. The clip test stays because the shared header's
      // `.cl-skip-link` uses the same off-screen idiom vertically, and because
      // a `clip: rect(0,0,0,0)` or `clip-path: inset(50%)` helper is the first
      // thing anyone adds when they need a screen-reader-only string.
     ...
     *
     * What this lab hides is a closed set, and one member of it is NOT
     * decorative. The decorative ones are the three `.pipe-arrow`s between the
     * decryption-pipeline cells (`× f →`, `mod p →`, `× F_p →`, each duplicated
     * by the `role="group"` label on the pipeline itself) and the shared
     * header's two `<svg>` marks.
     *
     * The exception is `.ring-legend`, which appears twice and is the COLOUR
     * KEY for the nine canvases — "+1", "−1", "0", "mod-q magnitude (gold)"
     * beside `.swatch` chips painted in those exact colours. It is fully
     * painted text that both oracles skip, so it is hand-measured in `gate.ts`'s
     * header instead of assumed. That it is `aria-hidden` at all is itself
     * recorded there as a finding: a screen-reader user gets the canvas's
     * one-line `aria-label` and no key to what its colours mean.
     *
     * Nothing on this page hides a VALUE. No coefficient, log line, status or
     * recovered message is inside an `aria-hidden` subtree — which was checked
     ...
     *
     * This lab has no SVG of its own — every figure it draws is a `<canvas>`.
     * The guard stays because the usual way the mistake arrives is exactly how
     * this lab builds its DOM: one enormous `innerHTML` template literal in
     * `main.ts`. Interpolating a string ARRAY into one makes JS join it with
     * commas, leaving a run of "," text nodes inside whatever element wraps it
     * — and inside a `<g>` those render nothing while a walk that measured them
     * would report a 1:1 failure describing ink that was never laid down.
     */
     ...
     *
     * THIS IS THE ONE LAB IN THE SWEEP WHERE THE BRANCH ACTUALLY FIRES. There
     * is no `html { background-color: … }` rule at all here: the page's entire
     * canvas is three stacked background-images on `<body>` — two radial washes
     * and a `linear-gradient(180deg, #060911, #090f18)` — and `<html>` paints
     * nothing. So `rootPaint.a` is 0, the fallback to `<body>` runs on every
     * scan, and the gradient sampler below is what turns those three layers
     * into the colour actually under the text. Without this, everything on a
     * page several screens tall would be measured against WHITE, in a lab whose
     * darkest surface is `#060911`.
     */
