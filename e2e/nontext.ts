 ...
 *
 * WHAT THIS LAB PUT THROUGH IT. The gate this replaces had a 1.4.11 check of
 * its own, `minControlBorderContrast`, and it took a SELECTOR ARGUMENT that was
 * called exactly twice — both times with `'#message-input'`. One element. The
 * page has fifteen buttons, and not one of them was ever measured. The check
 * itself was good work: it walked ancestor gradients and took the worst stop,
 * which is more than most in this fleet did. It was pointed at the one control
 * whose border (`#6d86b0`) carries a comment saying it was chosen for this
 * rule — measuring the place the rule is already kept.
 *
 ...
 *  - `<canvas>`. Nine of them carry every visual result this lab produces, and
 *    a canvas is one opaque element to every oracle here. Its pixels come from
 *    hard-coded hex in `main.ts`.
 *  - `.swatch`, the inline-styled colour chips in `.ring-legend` that are the
 *    KEY to those canvases. They are `<span>`s with a background and a
 *    1px `rgb(255 255 255 / 12%)` border, not controls, so the boundary pass
 *    below does not judge them.
 */
...
export async function auditNonText(page: Page, within = 'body *'): Promise<NonTextFailure[]> {
  return page.evaluate((rootSelector) => {
     ...
     *
     * The expensive part here is the KaTeX-rendered equation list, which
     * expands each formula into dozens of nested `<span>`s over a `<body>`
     * whose canvas is three stacked gradients. Without the caches the pass
     ...
     * theme. It is load-bearing on THIS page rather than defensive: `<body>`
     * has no background-color at all — its canvas is two radial gradients and a
     * linear one — and every `button` is filled with
     * `linear-gradient(180deg, #102b3d, #0b2232)`. Reading `background-color`
     * would report `rgba(0,0,0,0)` for both and judge every control against
     * white.
     */
